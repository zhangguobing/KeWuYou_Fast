...
import com.dtr.settingview.lib.R;
...
public class CheckItemViewV extends FrameLayout {
	...
	private void readAttrs(Context context, AttributeSet attrs) {
		if (null != attrs) {
			...
			if (a.hasValue(R.styleable.SettingViewItem_setting_view_check)) {
				Drawable drawable = a.getDrawable(R.styleable.SettingViewItem_setting_view_check);
				if (null != drawable) {
			...
			if (a.hasValue(R.styleable.SettingViewItem_setting_view_background)) {
				Drawable drawable = a.getDrawable(R.styleable.SettingViewItem_setting_view_background);
				if (null != drawable) {
			...
			if (a.hasValue(R.styleable.SettingViewItem_setting_view_drawable)) {
				Drawable drawable = a.getDrawable(R.styleable.SettingViewItem_setting_view_drawable);
				if (null != drawable) {
			...
			if (a.hasValue(R.styleable.SettingViewItem_setting_view_title)) {
				String title = a.getString(R.styleable.SettingViewItem_setting_view_title);
				if (!TextUtils.isEmpty(title)) {
					mTitle.setText(title);
				}
			}

			if (a.hasValue(R.styleable.SettingViewItem_setting_view_titleColor)) {
				ColorStateList colors = a.getColorStateList(R.styleable.SettingViewItem_setting_view_titleColor);
				if (null != colors) {
					mTitle.setTextColor(colors);
				}
			}

			if (a.hasValue(R.styleable.SettingViewItem_setting_view_titleSize)) {
				int textSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, a.getDimensionPixelSize(R.styleable.SettingViewItem_setting_view_titleSize, 16), getResources()
						.getDisplayMetrics());
				mTitle.setTextSize(textSize);
			}

			if (a.hasValue(R.styleable.SettingViewItem_setting_view_subTitle)) {
				String subTitle = a.getString(R.styleable.SettingViewItem_setting_view_subTitle);
				if (!TextUtils.isEmpty(subTitle)) {
					mSubTitle.setText(subTitle);
				}
			}

			if (a.hasValue(R.styleable.SettingViewItem_setting_view_subTitleColor)) {
				ColorStateList colors = a.getColorStateList(R.styleable.SettingViewItem_setting_view_subTitleColor);
				if (null != colors) {
					mSubTitle.setTextColor(colors);
				}
			}

			if (a.hasValue(R.styleable.SettingViewItem_setting_view_subTitleSize)) {
				int textSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, a.getDimensionPixelSize(R.styleable.SettingViewItem_setting_view_subTitleSize, 12), getResources()
						.getDisplayMetrics());
				mSubTitle.setTextSize(textSize);
			}

			if (a.hasValue(R.styleable.SettingViewItem_setting_view_clickable)) {
				mItemViewContainer.setClickable(a.getBoolean(R.styleable.SettingViewItem_setting_view_clickable, true));
			} else {
				mItemViewContainer.setClickable(true);
			}

			if (a.hasValue(R.styleable.SettingViewItem_setting_view_checked)) {
				boolean isChecked = a.getBoolean(R.styleable.SettingViewItem_setting_view_checked, false);
				if (isChecked) {
					mCheck.setVisibility(View.VISIBLE);
				} else {
					mCheck.setVisibility(View.GONE);
